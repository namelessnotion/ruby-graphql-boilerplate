...
RSpec.describe Services::WillnotdoNote, :aggregate_failures do
  it 'transitions the note to willnotdo' do
    note = Note.create(note: 'remember the milk')

    result = described_class.new(id: note.id).call

    expect(result.state).to eq('willnotdo')
  ...
  it 'raises when the note is already completed' do
    ...
    expect { described_class.new(id: note.id).call }
      .to raise_error(StateMachines::Sequel::FailedTransition, /failed to transition on willnotdo/)
